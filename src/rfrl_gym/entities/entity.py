...
class Entity(ABC):
    ...
    def __validate_entity(self):
        ...
        for channel in self.channels:
            if channel < 0 or channel >= self.num_channels:
                raise ValueError('Parameter "channels" of entity "{}" is not valid for the current gym environment.'.format(self.entity_label))
        # Validate onoff parameter.
        assert self.onoff[0] in [0,1], 'Parameter "onoff" of entity "{}" is not valid.'.format(self.entity_label)
        if (self.onoff[1] < 0 or not isinstance(self.onoff[1], int)) or (self.onoff[2] < 0 or not isinstance(self.onoff[2], int)):
            raise ValueError('Parameter "onoff" of entity "{}" is not valid.'.format(self.entity_label))
        # Validate start and stop parameters.
        if (not isinstance(self.start, int)):
            raise ValueError('Parameter "start" of entity "{}" is not valid.'.format(self.entity_label))
        if (not isinstance(self.stop, int)) and (self.stop != np.inf):
            raise ValueError('Parameter "stop" of entity "{}" is not valid.'.format(self.entity_label))       

        self._validate_self()

    @abstractmethod
    def _validate_self(self):
        raise NotImplementedError('Necessary entity function _validate_self() not implemented for entity type {}'.format(type(self).__name__))
    ...
    def _get_action(self):
        raise NotImplementedError('Necessary entity function _get_action() not implemented for entity type {}'.format(type(self).__name__))
    ...
    @abstractmethod
    def _reset(self):
        raise NotImplementedError('Necessary entity function _reset() not implemented for entity type {}'.format(type(self).__name__))
